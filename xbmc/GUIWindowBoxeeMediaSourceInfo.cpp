...
#define CONTROL_REMOVE_BUTTON         56
...
#define CONTROL_SCAN_MONITORED_BUTTON 84
...
bool CGUIWindowBoxeeMediaSourceInfo::OnMessage(CGUIMessage& message)
{
  ...
  case GUI_MSG_CLICKED:
  {
    ...
    else if (senderId == CONTROL_ADD_UPDATE_BUTTON)
    {
      ...
      if (((CGUIRadioButtonControl*) GetControl(CONTROL_SCAN_PRIVATE_BUTTON))->IsSelected())
      {
        source.scanType = CMediaSource::SCAN_TYPE_PRIVATE;
      }
      else if (((CGUIRadioButtonControl*) GetControl(CONTROL_SCAN_ONCE_BUTTON))->IsSelected())
      {
        source.scanType = CMediaSource::SCAN_TYPE_ONCE;
      }
      else if (((CGUIRadioButtonControl*) GetControl(CONTROL_SCAN_DAILY_BUTTON))->IsSelected())
      {
        source.scanType = CMediaSource::SCAN_TYPE_DAILY;
      }
      else if (((CGUIRadioButtonControl*) GetControl(CONTROL_SCAN_MONITORED_BUTTON))->IsSelected())
      {
      ...
      if (source.scanType !=  CMediaSource::SCAN_TYPE_PRIVATE && source.scanType !=  CMediaSource::SCAN_TYPE_ONCE &&
          source.scanType !=  CMediaSource::SCAN_TYPE_DAILY && source.scanType !=  CMediaSource::SCAN_TYPE_MONITORED)
      {
      ...
      if (m_editedSourceName != "")
      {
        ...
        g_application.m_guiDialogKaiToast.QueueNotification("", "", g_localizeStrings.Get(51038), 5000); 
      }
      else
      {
        sourceList.addSource(source);
        g_application.m_guiDialogKaiToast.QueueNotification("", "", g_localizeStrings.Get(51037), 5000);  
      }
...
//      BOXEE::Boxee::GetInstance().GetMetadataEngine().MarkFolderTreeNew(_P(m_sourcePath));
      ...
      g_application.m_guiDialogKaiToast.QueueNotification("", "", g_localizeStrings.Get(51043), 5000);
    }
  }
  break;
  }
...
void CGUIWindowBoxeeMediaSourceInfo::OnInitWindow()
{
  ...
  if (m_editedSourceName != "")
  {
    ...
    SET_CONTROL_LABEL(CONTROL_ADD_UPDATE_BUTTON, g_localizeStrings.Get(54200));
  ...
  else
  {
    ...
    SET_CONTROL_LABEL(CONTROL_ADD_UPDATE_BUTTON, g_localizeStrings.Get(54201));
    ...
    CURL url(m_sourcePath);      
    if (url.GetProtocol() == "smb")
    {
      name += " on ";
      name += url.GetHostName();
    }
  ...
  {
    SET_CONTROL_LABEL(CONTROL_LOCATION_LABEL, shortenURL(_P(m_sourcePath)));
...
CStdString CGUIWindowBoxeeMediaSourceInfo::shortenURL(CStdString path)
{
  ...
  CURL url(result);
  url.GetURLWithoutUserDetails(result);
