...
#define ID3V2HEADERSIZE 10
  ...
  unsigned char* xing;
  unsigned char* vbri;
  unsigned char buffer[SCANSIZE + 64];

  memset(buffer,0,SCANSIZE + 64);
  ...
  /* Check if the file has an ID3v1 tag */
  int64_t nFileLen = file.GetLength();
  if (nFileLen <= 128)
  {
    CLog::Log(LOGWARNING, "file len < 128 on mp3 tag reading. file: <%s>", strFileName.c_str());
    return 0;
  }
