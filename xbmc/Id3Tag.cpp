...
{
  CLog::Log(LOGDEBUG,"ID3: trying to load from <%s>", strFile.c_str());
...
bool CID3Tag::Parse()
{
  ...
  if (!m_tag)
  {
    CLog::Log(LOGERROR,"ID3: no tag to parse!");
    return false;
  }
  ...
  if (bFound && !CUtil::ThumbExists(strCoverArt))
  {
    ...
    // we check that size is larger than 128 as sanity to avoid 
    // crash in ImageLib on corrupt images (example, nBufSize = 4)
    if (pPic != NULL && nBufSize > 128) 
    {
...
{
  CLog::Log(LOGDEBUG,"ID3: writing <%s>", strFile.c_str());
  if (!m_dll.Load()) return false;
...
{
  if (!m_dll.Load()) return StringUtils::EmptyString;
