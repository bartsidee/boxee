...
VECSOURCES& CGUIViewStateWindowVideoFiles::GetSources()
{
  ...
  if (CPluginDirectory::HasPlugins("video") && g_advancedSettings.m_bVirtualShares)
  {
    ...
    if (CUtil::GetMatchingSource(share.strName, videoSources, bIsSourceName) < 0)
	  g_settings.AddShare("video",share);
  }
  return *g_settings.GetSourcesFromType("video");
}
...
//      else
      AddSortMethod(SORT_METHOD_LABEL_WITH_SHARES, 551, LABEL_MASKS("%L", "%I", "%L", ""));  // FileName, Size | Foldername, empty
      AddSortMethod(SORT_METHOD_DATE_WITH_SHARES, 552, LABEL_MASKS("%L", "%J", "%L", "%J"));  // FileName, Date | Foldername, Date
      AddSortMethod(SORT_METHOD_EPISODE, 551, LABEL_MASKS()); // Preformated
...
VECSOURCES& CGUIViewStateWindowVideoBoxeeDb::GetShares()
{
  return g_settings.m_videoSources;
}

void CGUIViewStateWindowVideoBoxeeDb::SaveViewState()
{
...
CGUIViewStateWindowVideoRssDirectory::CGUIViewStateWindowVideoRssDirectory(const CFileItemList& items) : CGUIViewStateWindowVideo(items)
{
  CLog::Log(LOGDEBUG, "CGUIViewStateWindowVideoRssDirectory, VIEWSTATE");
  if (g_guiSettings.GetBool("filelists.ignorethewhensorting")) {
...
void CGUIViewStateWindowVideoRssDirectory::SaveViewState()
{
  SaveViewToDb(m_items.m_strPath, WINDOW_VIDEO_FILES);
}

CGUIViewStateWindowVideoRssItems::CGUIViewStateWindowVideoRssItems(const CFileItemList& items) : CGUIViewStateWindowVideo(items)
{
  CLog::Log(LOGDEBUG, "CGUIViewStateWindowVideoRssItems, VIEWSTATE");
  if (g_guiSettings.GetBool("filelists.ignorethewhensorting"))
    AddSortMethod(SORT_METHOD_LABEL_IGNORE_THE, 551, LABEL_MASKS("%K", "%I", "%L", ""));  // Titel, Size | Foldername, empty
  else
    AddSortMethod(SORT_METHOD_LABEL, 551, LABEL_MASKS("%K", "%I", "%L", ""));  // Titel, Size | Foldername, empty
  
  AddSortMethod(SORT_METHOD_RSS_ITEMS, 552, LABEL_MASKS("%K", "%I", "%L", ""));  // Titel, Size | Foldername, empty
  //AddSortMethod(SORT_METHOD_DATE, 552, LABEL_MASKS("%K", "%J", "%L", "%J"));  // Titel, Date | Foldername, Date

  SetSortMethod(SORT_METHOD_RSS_ITEMS);
  SetSortOrder(SORT_ORDER_ASC);

  LoadViewState(items.m_strPath, WINDOW_VIDEO_FILES);
}

void CGUIViewStateWindowVideoRssItems::SaveViewState()
{
  SaveViewToDb(m_items.m_strPath, WINDOW_VIDEO_FILES);
}
