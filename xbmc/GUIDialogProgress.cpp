...
#define CONTROL_PROGRESS_BAR2 21
...
void CGUIDialogProgress::Close(bool bForce)
{
  CGUIDialog::Close(bForce);
  m_strId.clear();
}

void CGUIDialogProgress::StartModal(const std::string &strId)
{
...
void CGUIDialogProgress::OnWindowLoaded()
{
  ...
  //second progress bar to enable easy placing under different conditions
  control = GetControl(CONTROL_PROGRESS_BAR2);
  if (control && control->GetControlType() == CGUIControl::GUICONTROL_PROGRESS)
  {
    // make sure we have the appropriate info set
    CGUIProgressControl *progress = (CGUIProgressControl *)control;
...
bool CGUIDialogProgress::OnFileCallback(void* pContext, int ipercent, float avgSpeed)
{
  SetPercentage(ipercent);
  return !IsCanceled();
}

void CGUIDialogProgress::SetOperationID(const std::string &strId)
{
  m_strId = strId;
}

const std::string &CGUIDialogProgress::GetOperationID()
{
  return m_strId;
}

void CGUIDialogProgress::EndProgressOperation(const std::string &strId)
{
  CGUIDialogProgress* pDlgProgress = (CGUIDialogProgress*)g_windowManager.GetWindow(WINDOW_DIALOG_PROGRESS);
  if (pDlgProgress && !pDlgProgress->m_strId.empty() && pDlgProgress->m_strId == strId)
  {
    pDlgProgress->Close();
    pDlgProgress->m_strId.clear();
  }
}
