  ...
  case GUI_MSG_CLICKED:
    {
                                                    ...
                                                    message.GetParam1() == ACTION_MOUSE_LEFT_CLICK))
      {
        ...
        if (pList)
        {
          ...
          CLog::Log(LOGDEBUG,"CGUIDialogVisualisationPresetList::OnMessage, GUI_MSG_WINDOW_INIT, VISUAL, current vis = %d", m_currentPreset);
          if (iItem == m_currentPreset) {
            if (m_pVisualisation)
              m_pVisualisation->OnAction(CVisualisation::VIS_ACTION_LOCK_PRESET, (void *)&iItem);
          }
          else {
  ...
  case GUI_MSG_WINDOW_INIT:
    {
      ...
      CLog::Log(LOGDEBUG,"CGUIDialogVisualisationPresetList::OnMessage, GUI_MSG_WINDOW_INIT, VISUAL, current vis = %d", m_currentPreset);
      CGUIMessage msg2(GUI_MSG_ITEM_SELECT, GetID(), CONTROL_LIST, m_currentPreset);
...
void CGUIDialogVisualisationPresetList::SetVisualisation(CVisualisation *pVisualisation)
{
  ...
  if (m_pVisualisation)
  {
    ...
    CLog::Log(LOGDEBUG,"CGUIDialogVisualisationPresetList::SetVisualisation, GUI_MSG_WINDOW_INIT, VISUAL, current vis = %d", m_currentPreset);
    if (presets)
    {
      //clear filelist
