...
void CGUIViewControl::SetCurrentViewByID(int viewId, bool focusToCurrentView)
{
  ...
  int newView = GetView(VIEW_TYPE_NONE, viewId);
...
// returns the next viewmode in the cycle
int CGUIViewControl::GetNextViewID(int direction) const
{
  if (!m_visibleViews.size())
    return 0; // no view modes :(
...
int CGUIViewControl::GetCurrentViewID() const
{
  if (!m_visibleViews.size())
    return 0; // no view modes :(

  int viewNumber = (m_currentView) % (int)m_visibleViews.size();
  if (viewNumber < 0) viewNumber += m_visibleViews.size();
  CGUIBaseContainer *currentView = (CGUIBaseContainer *)m_visibleViews[viewNumber];
  if (currentView)
  {
