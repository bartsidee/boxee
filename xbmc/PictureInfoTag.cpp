...
ExifInfo_t& CPictureInfoTag::GetEXIFfInfo()
{
  return m_exifInfo;
}

IPTCInfo_t& CPictureInfoTag::GetIPTCIInfo()
{
  return m_iptcInfo;
}
...
{  
  if (info.Equals("label")) return SLIDE_FILE_LABEL;
  else if (info.Equals("filename")) return SLIDE_FILE_NAME;
  ...
  else if (info.Equals("showinfo")) return SLIDE_SHOWINFO;
  else if (info.Equals("description")) return SLIDE_DESCRIPTION;
  else if (info.Equals("isloadingLocal")) return SLIDE_IS_LOADING_LOCAL;
  else if (info.Equals("isloadingRemote")) return SLIDE_IS_LOADING_REMOTE;
  else if (info.Equals("canshare")) return SLIDE_CAN_SHARE;
