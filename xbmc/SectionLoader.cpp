...
LibraryLoader *CSectionLoader::LoadDLL(const CStdString &dllname, bool bDelayUnload /*=true*/, bool bLoadSymbols /*=false*/)
{
  ...
  typedef const char * (*GetSigFuncType) (void);
#ifdef _LINUX
  CStdString path=CSpecialProtocol::TranslatePath(dllname);
  CStdString sigFuncName = CUtil::GetFileName(path)+"__sig";
  sigFuncName.Replace('.', '_');
  sigFuncName.Replace('-', '_');
  GetSigFuncType sigFunc = (GetSigFuncType)dlsym(RTLD_DEFAULT, sigFuncName.c_str());
  if (sigFunc)
  {
    std::string sig = sigFunc();
    if (!CUtil::CheckFileSignature(path, sig))
    {
      CLog::Log(LOGERROR,"dll verification failed (%s)", dllname.c_str());
      return NULL;
    }
  }
#endif
