 ...
 */
bool CUtil::RunInBG(IRunnable* pJob)
{
...
bool CUtil::RemovePasswordFromPath(CStdString &path)
{
  ...
  if (reg.RegComp("(//.*?@)"))
  {
    if (reg.RegFind(path.c_str()) >= 0)
    {
      char *rep = reg.GetReplaceString("\\1");
      if (rep)
      {
        path.Replace(rep, "//******@");
