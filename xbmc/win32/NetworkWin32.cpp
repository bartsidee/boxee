...
bool CNetworkInterfaceWin32::IsConnected(void)
{
  CStdString strIP = m_adapter.IpAddressList.IpAddress.String;
  return (strIP != "0.0.0.0");
}

CStdString CNetworkInterfaceWin32::GetMacAddress(void)
{
  CStdString result = "";
  // m_adapter.Address is a byte array and should be a 6 byte mac
  if( 6 == m_adapter.AddressLength )
  {
    BYTE* s = m_adapter.Address;
    result.Format("%.2x:%.2x:%.2x:%.2x:%.2x:%.2x", s[0], s[1], s[2], s[3], s[4], s[5]);
  }
  return result;
}
...
CStdString CNetworkInterfaceWin32::GetCurrentBroadcastAddress(void)
{
  // Given the netmask and the ip address, calculate the broadcast address
  struct in_addr ip, nm, bc;
  inet_aton( m_adapter.IpAddressList.IpAddress.String, &ip );
  inet_aton( m_adapter.IpAddressList.IpMask.String, &nm );
  bc.s_addr = (ip.s_addr & nm.s_addr) | ~(nm.s_addr);
  return inet_ntoa( bc );
}
...
void CNetworkWin32::queryInterfaceList()
{
  ...
  if ((GetAdaptersInfo(adapterInfo, &ulOutBufLen)) == NO_ERROR) 
  {
    ...
    while (adapter) 
    {
      m_interfaces.push_back(CNetworkInterfacePtr(new CNetworkInterfaceWin32(this, *adapter)));
