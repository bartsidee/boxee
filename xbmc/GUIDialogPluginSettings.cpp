...
#define CONTROL_HEADER_LABEL            52
...
bool CGUIDialogPluginSettings::OnAction(const CAction &action)
{
        ...
        if (strcmp(type, "text") == 0)
        {
          ...
          if (CGUIDialogKeyboard::ShowAndGetInput(value, ((CGUIButtonControl*) control)->GetLabel(), true, bHidden))
          {
            ...
            else
            {
              ...
              ((CGUIButtonControl*) control)->SetLabel2(hiddenPassword);
              
              const char *id = setting->Attribute("id");
              CStdString key(id);
              dialogValuesMap[key] = value;
            }
          }
        }
...
bool CGUIDialogPluginSettings::SaveSettings(void)
{
  ...
  while (setting)
  {
    ...
    // special treat to "header" type
    if (strcmpi(type, "header") == 0)
    {
      CGUILabelControl *pOriginalHeaderLabel = (CGUILabelControl *)GetControl(CONTROL_HEADER_LABEL);

      if (pOriginalHeaderLabel)
        value = pOriginalHeaderLabel->GetLabel();
    }
...
{  
  ...
  if (pOriginalHeaderLabel)
    pOriginalHeaderLabel->SetVisible(false);
  ...
  {
    CLog::Log(LOGERROR, "CGUIDialogPluginSettings::CreateControls -m_setting Plugin Is Null ");
    return;
  ...
  {   
    ...
    {      
      ...
      ((CGUIButtonControl *)pControl)->SetLabel(BoxeeUtils::TranslateStringById(label));
      if (id)
      {
        ...
        if (option)
        {
          ...
          if(bHidden)
          {
            ...
            ((CGUIButtonControl *)pControl)->SetLabel2(hiddenPassword);
          ...
          {
            ((CGUIButtonControl *)pControl)->SetLabel2(BoxeeUtils::TranslateStringById(value));
          }
        }
        else
        {
    ...
    {
       ((CGUILabelControl *)pOriginalHeaderLabel)->SetLabel(BoxeeUtils::TranslateStringById(label));
       ((CGUILabelControl *)pOriginalHeaderLabel)->SetVisible(true);
    }
    }
    if (pControl)
    {
...
void CGUIDialogPluginSettings::SetDefaults()
{
  ...
  if (!m_settings.GetPluginRoot())
  {
    CLog::Log(LOGDEBUG,"CGUIDialogPluginSettings::SetDefaults - PluginRoot is NULL");
    return;
  }
  TiXmlElement *setting = m_settings.GetPluginRoot()->FirstChildElement("setting");
  while (setting)
...
void CGUIDialogPluginSettings::SetSettings(const CBasicSettings &settings)
{
  m_settings = settings;  
}

CStdString CGUIDialogPluginSettings::GetValue(const CStdString &strKey)
{
  CStdString retVal =  "";
  map<CStdString, CStdString>::const_iterator iter = dialogValuesMap.find(strKey);
  
  if(iter != dialogValuesMap.end())
  {
    retVal = (dialogValuesMap[strKey]);
  }
  else
  {
    retVal = m_settings.Get(strKey);
  }
  
  return retVal;
}

CStdString CGUIDialogPluginSettings::GenerateXmlSettingsStr(void)
{
