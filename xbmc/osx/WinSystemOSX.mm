...
bool CWinSystemOSX::CreateNewWindow(const CStdString& name, bool fullScreen, RESOLUTION_INFO& res, PHANDLE_EVENT_FUNC userFunction)
{
  ...
  NSString *string;
  string = [ [ NSString alloc ] initWithUTF8String:"Boxee" ];
...
{  
  ...
  // make sure this is a valid screen (user might disconnected ext monitor)
  int numDisplays = [[NSScreen screens] count];
  if (res.iScreen >= numDisplays)
  {
    m_nWidth      = g_settings.m_ResInfo[RES_DESKTOP].iWidth;
    m_nHeight     = g_settings.m_ResInfo[RES_DESKTOP].iHeight;
  }
  else  
  {
  m_nWidth      = res.iWidth;
  ...
  else
  {
    ...
    [ [lastView window] setContentSize:NSMakeSize(m_nWidth, m_nHeight) ];
...
void CWinSystemOSX::UpdateResolutions()
{
  ...
  // Copy settings from all existing displays to newly detected ones
  if (m_bSettingsLoaded)
  {	  
    while(it1 != g_settings.m_ResInfo.end() && it2 != m_tempResInfo.end())
    {
	  *it1 = *it2;
	  it1++;
	  it2++;
