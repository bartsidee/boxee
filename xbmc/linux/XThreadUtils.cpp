...
// Returns success
bool QueryStackStatus(size_t* stack_size, size_t* used_stack, size_t* stack_left) {
#if defined( _LINUX) && !defined(__APPLE__)
  int my_pid = pthread_self();
  pthread_attr_t attr;
  if (pthread_getattr_np (pthread_self (), &attr) != 0)
    return false;
  void *stack_start;
  if (pthread_attr_getstack (&attr, &stack_start, stack_size) != 0)
    return false;

  size_t current_addr = (size_t)(void*)&current_addr;
  *stack_left = current_addr - (size_t)stack_start;
  *used_stack = *stack_size - *stack_left;
  return true;
#else
  return false;
#endif
