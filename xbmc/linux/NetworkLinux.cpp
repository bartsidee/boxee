...
bool CNetworkInterfaceLinux::IsWireless(void)
{
#ifdef __APPLE__
  return false;
#else
...
bool CNetworkInterfaceLinux::IsEnabled(void)
{
   struct ifreq ifr;
...
bool CNetworkInterfaceLinux::IsConnected(void)
{
   struct ifreq ifr;
...
CStdString CNetworkInterfaceLinux::GetMacAddress(void)
{
   CStdString result = "";

#ifdef __APPLE__
  
  CFMutableDictionaryRef matchingDict = IOBSDNameMatching(kIOMasterPortDefault, 0, m_interfaceName.c_str());
  if (matchingDict != NULL) 
  {
    ...
    kern_return_t kernResult = IOServiceGetMatchingServices(kIOMasterPortDefault, matchingDict, &matchingServices);
     if (kernResult == KERN_SUCCESS)
     {
       ...
       while ( !found && (intfService = IOIteratorNext(matchingServices)) )
       {
...
CStdString CNetworkInterfaceLinux::GetCurrentBroadcastAddress(void)
{
  CStdString result = "";
  ...
  strcpy(ifr.ifr_name, m_interfaceName.c_str());
  ifr.ifr_addr.sa_family = AF_INET;
  if (ioctl(m_network->GetSocket(), SIOCGIFBRDADDR, &ifr) >= 0)
  {
    result = inet_ntoa( ((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr );
  }
  return result;
...
void CNetworkLinux::queryInterfaceList()
{
   ...
   for(cur = list; cur != NULL; cur = cur->ifa_next)
   {
     ...
     // Add the interface.
     m_interfaces.push_back(CNetworkInterfacePtr(new CNetworkInterfaceLinux(this, cur->ifa_name)));
   }
   ...
   while (getdelim(&line, &linel, '\n', fp) > 0)
   {
    ...
    // search where the word begins
      ...
      if (ioctl(GetSocket(), SIOCGIFHWADDR, &ifr) >= 0 && ifr.ifr_hwaddr.sa_family == ARPHRD_ETHER)
      {
         ...
         m_interfaces.push_back(CNetworkInterfacePtr(new CNetworkInterfaceLinux(this, interfaceName)));
      }
   }
     free(line);
   fclose(fp);
