...
#if defined (HAS_DVD_DRIVE) && !defined(__APPLE__)
  // special case for opening the cdrom device
  // in case that we open a log file we should avoid the "cdrom" check since we dont want to acquire the
  // mediadevice mutex.
  if (strstr(lpFileName, "boxee.log") == NULL &&
		  strcmp(lpFileName, MEDIA_DETECT::CLibcdio::GetInstance()->GetDeviceFileName())==0)
...
BOOL DeleteFile(LPCTSTR lpFileName)
{
  ...
  if (errno == EACCES) {
    ...
    strLower.MakeLower();
    CLog::Log(LOGDEBUG,"%s - cant delete file <%s>. trying lower case <%s>", __FUNCTION__, lpFileName, strLower.c_str());
