...
void CGUIWindowVideoInfo::SetMovie(const CFileItem *item)
{
  ...
  *m_movieItem = *item;
  ...
  if (m_movieItem->m_strPath == "") 
  {
	  m_movieItem->SetProperty("isplayable","0");
  }
  else 
  {
	  m_movieItem->SetProperty("isplayable","1");
  }
  ...
  loader.LoadItem(m_movieItem.get(), false);
}

void CGUIWindowVideoInfo::Update()
{
	
  if ((m_movieItem->IsHD() || m_movieItem->IsSmb()) && !m_movieItem->m_bIsFolder && !m_movieItem->m_bIsShareOrDrive && 
	!m_movieItem->m_strPath.IsEmpty() && !m_movieItem->GetPropertyBOOL("MetaDataExtracted") && 
	g_application.IsPathAvailable(m_movieItem->m_strPath)) {
	  CDVDFileInfo::GetFileMetaData(m_movieItem->m_strPath, m_movieItem.get());
  }
	
  CLog::Log(LOGDEBUG,"CGUIWindowVideoInfo::Update");
  m_movieItem->Dump();
  ...
  CGUIMessage msg2(GUI_MSG_LABEL_BIND, GetID(), CONTROL_LIST, 0, 0, m_castList);
  OnMessage(msg2);
