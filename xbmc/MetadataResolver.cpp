...
bool CMetadataResolver::CheckPath(const std::string& strPath)
{
  ...
  if (!CURL::IsFileOnly(strPath) && !CUtil::IsHD(strPath) && !CUtil::IsSpecial(strPath))
  {
...
int CMetadataResolver::Resolve(BXMetadataScannerJob * pJob, std::vector<BXFolder*>& vecResults )
{
  if (!pJob || pJob->GetFolder() == NULL || pJob->GetFolder()->GetPath().empty()) {
    CLog::Log(LOGERROR, "CMetadataResolver::Resolve, could not resolve file, input is NULL");
    return RESOLVER_FAILED;
  }

  const BXFolder* pInputFolder = pJob->GetFolder();

  CLog::Log(LOGDEBUG, "CMetadataResolver::Resolve, resolving folder, path = %s (resolver)", pInputFolder->GetPath().c_str());
  ...
  else if (folderType == MEDIA_ITEM_TYPE_VIDEO_FOLDER)
  {
    ...
    CLog::Log(LOGERROR, "CMetadataResolver::Resolve, could not resolve folder, unrecognized type: %s", folderType.c_str());
...
int CMetadataResolver::ResolveMusicFolder(BXMetadataScannerJob* pJob)
{
  ...
  CStdString strFolderPath = pJob->GetFolder()->GetPath();
  if (strFolderPath == "") {
  ...
  return CMetadataResolverMusic::ResolveMusicFolder(strFolderPath);
...
/**
  ...
  of retreived files and folders with the list retrieved from the database
...
void CMetadataResolver::PurgeDeletedVideosFromDatabase(const CStdString& strFolderPath, const CFileItemList& items)
{
  ...
  BOXEE::Boxee::GetInstance().GetMetadataEngine().GetVideosByFolder(mapDatabaseCache, strFolderPath);
  ...
  for (int i = 0; i < items.Size(); i++)
  {
    ...
    if (pItem->m_bIsFolder) 
    {
      ...
      // This is a VERY VERY VERY ugly patch, but since the slash/backslash issue is alittle bit complicated
      // and already patch base, this is the best i ca do.
...
{
  if (CMetadataResolverVideo::IsDVDFolder(items)) 
  {
    bool bResult = false;
    ...
    // Create and initialize appropritate video content
    ...
    if (CMetadataResolverVideo::ResolveVideo(context, pMetadata))
    {
      ...
      else
      {
        ...
        {
          CStdString strItemPath = items[i]->m_strPath;
    ...
    {
      MDE.MarkFolderUnresolved(strFolderPath);
    }
...
bool CMetadataResolver::ResolveTwoVideoFolder(std::vector<CFileItemPtr>& items)
{
  ...
  if (iNumVideoItems == 2) 
  {
    ...
    CVideoFileContext context1(items[0]->m_strPath);
    context1.LoadNameFromPath();

    CVideoFileContext context2(items[1]->m_strPath);
    context2.LoadNameFromPath();

    if (context1.bMovie && context2.bMovie)
    {
      double relevance = fstrcmp(items[0]->m_strPath.c_str(), items[1]->m_strPath.c_str(), 0.0f);
      if (relevance > 0.8) 
      {
        return true;
      }
...
int CMetadataResolver::ResolveVideoFolder (BXMetadataScannerJob* pJob)
{
  ...
  CLog::Log(LOGDEBUG, "CMetadataResolver::ResolveVideoFolder, RESOLVER, Resolving video folder, path = %s, parent folder = %s, parent folder name = %s (resolver) (videoresolver)",
      strFolderPath.c_str(), folderContext.strEffectiveParentPath.c_str(), folderContext.strName.c_str());
...
//  }
  ...
  for (size_t i=0; i < vecVideoFiles.size() && !m_bStopped; i++)
  {
    ...
    if (CMetadataResolverVideo::IsSample(strPath))
    {
    ...
    // files with RAR extension shouldn't be resolved.
    if ((context.iSize < g_guiSettings.GetInt("myvideos.videosize") * 1024 * 1024) || pItem->IsRAR())
    {
      MDE.UpdateVideoFileStatus(strPath, STATUS_UNRESOLVED);
      continue;
    }
    ...
    else
    {
      ...
      MDE.UpdateVideoFileStatus(context.strPath, STATUS_UNRESOLVED);
    }
  ...
  CLog::Log(LOGDEBUG, "CMetadataResolver::ResolveVideoFolder, Updating folder status, path = %s, items = %u, resolved = %d (resolver) (videoresolver)",
          strFolderPath.c_str(), vecVideoFiles.size(), iResolvedItems);

  if (vecVideoFiles.size() > iResolvedItems)
  {
    // Mark folder as unresolved
    BOXEE::Boxee::GetInstance().GetMetadataEngine().MarkFolderUnresolved(strFolderPath);
    return RESOLVER_FAILED;
  }
  else 
  {
