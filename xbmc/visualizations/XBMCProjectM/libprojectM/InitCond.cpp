...
param->matrix_flag = false;
  ...
  if ( param->type == P_TYPE_INT) {
      // BOXEE
      // This ugly fix Zvisha added to prevent crush when illigal value is set into wave_mode in function 
      // Parser::parse_init_cond()
      if (init_val.int_val > param->upper_bound.int_val) {
        init_val.int_val = param->upper_bound.int_val;
      }
      // End BOXEE
 	assert(param->engine_val);
