...
void CPlayListPlayer::PlayNext(int offset, bool bAutoPlay)
{
  ...
  if (iSong < playlist.size() && !playlist[iSong]->IsVideo() && playlist[iSong]->IsAudio() && g_windowManager.GetActiveWindow() == WINDOW_FULLSCREEN_VIDEO)
...
void CPlayListPlayer::Play(int iSong, bool bAutoPlay /* = false */, bool bPlayPrevious /* = false */)
{
  ...
  if (m_iCurrentPlayList == PLAYLIST_MUSIC)
  {
  ...
  bool bPlaybackOk = g_application.PlayFile(*item, bAutoPlay);
  
  int nFallback = 0;
  while (!bPlaybackOk && nFallback < (int)item->m_fallbackPaths.size())
  {
    item->m_strPath = item->m_fallbackPaths[nFallback];
    item->SetContentType(""); // next call will cause the content type being queried off the actual url
    item->GetContentType(true);
    bPlaybackOk = g_application.PlayFile(*item, bAutoPlay);
    nFallback++;
  }
