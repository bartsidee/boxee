...
bool CSpinExSettingControl::OnClick()
{
  ...
  {
    CStdString setting = m_pSetting->GetSetting();
    // ugly hack - video resolution should only be changed by the "test" button
    if (m_pSetting->GetType() != SETTINGS_TYPE_STRING && setting != "videoscreen.resolution")
    {
