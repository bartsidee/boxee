...
CStdString CLangInfo::TempUnitToStr(TEMP_UNIT  unit) const
{
  CStdString  unitStr;
  ...
  case TEMP_UNIT_ROMER:
  {
    unitStr = "Ro";
    break;
  }
  case TEMP_UNIT_DELISLE:
  {
    unitStr = "De";
    break;
  }
  case TEMP_UNIT_NEWTON:
  {
    unitStr = "N";
    break;
  }
...
CStdString CLangInfo::TimeFormatToPattern(const CStdString &timeformat) const
{
  if (timeformat == g_localizeStrings.Get(54502))
  {
	return g_localizeStrings.Get(54500);
  }
  else
  {
	return g_localizeStrings.Get(54501);
  }
}
...
bool CLangInfo::Load(const CStdString& strFileName)
{
  if (g_application.IsCurrentThread())
    return LoadInternal(strFileName);
  else
    return g_application.getApplicationMessenger().LoadLangInfo(strFileName);
}

bool CLangInfo::LoadInternal(const CStdString& strFileName)
{
  SetDefaults();
  ...
  if (pRegions && !pRegions->NoChildren())
  {
    ...
    while (pRegion)
    {
      ...
      if (pTime && !pTime->NoChildren())
      {
        ...
        if ((strcmp(pTime->Attribute("symbolAM"),"") != 0)  && (strcmp(pTime->Attribute("symbolPM"),"") != 0))
        {
    ...
    SetTempUnit(g_guiSettings.GetString("locale.tempscale"));

    // it is ugly,but at this point the strings.xml file wasnt loaded yet
    const CStdString& pattern = g_guiSettings.GetString("locale.timeformat");
...
void CLangInfo::SetTimeFormatFromPatterns(const CStdString& pattern)
{
  if (pattern.Equals(g_localizeStrings.Get(54500)))
  {
	m_strTimeFormat =  g_localizeStrings.Get(54502);
  }
  else
  {
	m_strTimeFormat = g_localizeStrings.Get(54503);
  }
...
//	  return m_currentRegion->m_strMeridiemSymbols[symbol];
	  return m_strMeridiemSymbols[symbol];
}
...
void CLangInfo::GetTempUnits(CStdStringArray& array)
{
  array.push_back("F");
  array.push_back("K");
  array.push_back("C");
...
void CLangInfo::GetTimeFormats(CStdStringArray& array)
{
/*  for (ITMAPREGIONS it=m_regions.begin(); it!=m_regions.end(); ++it)
  {
    CStdString timeFormat=it->second.m_strTimeFormat;

    bool exist = false;
    // check if the time format is already exist in the array
	for (int i = 0 ; i < array.size(); i++)
	{
	  if (timeFormat == array[i])
	  {
		exist = true;
		break;
	  }
	...
	if (!exist)
	{
	  array.push_back(timeFormat);
	}
  }
  */
  array.push_back(g_localizeStrings.Get(54500));
  array.push_back(g_localizeStrings.Get(54501));
}
