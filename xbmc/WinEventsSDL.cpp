...
#elif defined(_LINUX)
      if( ProcessLinuxShortcuts( event ) )
      {
        ret = true;
	break;
      }      
...
bool CWinEventsSDL::ProcessOSXShortcuts(SDL_Event& event)
{
  ...
  if (cmd && event.key.type == SDL_KEYDOWN)
  {
    ...
    case SDLK_l: // CMD-l to toggle keyboards
...
bool CWinEventsSDL::ProcessLinuxShortcuts(SDL_Event& event)
{
  ...
  if( ctrl && event.key.type == SDL_KEYDOWN )
  {
    static CAction action;
      
    switch( event.key.keysym.sym )
    {
      case SDLK_v: // Crtl-v to paste
	      action.id = ACTION_PASTE;
 	      g_application.OnAction( action );
	      return true;
      default:
