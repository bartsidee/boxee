...
bool CMediaSource::IsCountryAllowed() const
{
  if (m_country.IsEmpty())
    return true;
  
  return CUtil::IsCountryAllowed(m_country, m_countryAllow);
}

bool CMediaSource::IsAllowed() const
{
  if (m_adult && !CUtil::IsAdultAllowed())
  {
    return false;
  }
...
bool CMediaSource::IsPrivate() const
{
  return (m_iScanType == SCAN_TYPE_PRIVATE);
}
