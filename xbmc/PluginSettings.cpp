...
bool CPluginSettings::Load(const TiXmlDocument &xmlDoc)
{

  if (xmlDoc.Error())
    return false;

  m_pluginXmlDoc = xmlDoc;
  m_userXmlDoc = xmlDoc;

  return true;
}

bool CPluginSettings::Load(const CStdString &strSettings)
{
  m_pluginXmlDoc.Parse(strSettings.c_str());
  if (m_pluginXmlDoc.Error())
  {
    CLog::Log(LOGERROR, "Unable to load: %s, Line %d\n%s", strSettings.c_str(), m_pluginXmlDoc.ErrorRow(), m_pluginXmlDoc.ErrorDesc());
    return false;
  }
  m_userXmlDoc.Parse(strSettings.c_str());
