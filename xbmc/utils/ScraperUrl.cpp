...
bool CScraperUrl::Get(const SUrlEntry& scrURL, string& strHTML, XFILE::CFileCurl& http)
{
  ...
  if (!scrURL.m_cache.IsEmpty())
  {
    ...
    if (XFILE::CFile::Exists(strCachePath))
    {
      ...
      char* temp = new char[iLength + 1];
      memset(temp, 0, iLength+1);
      
      file.Read(temp,iLength);
      
      // The append function expects length as a second parameter, and not the pointer ot the end of array
      // as it appeared in the previous code
      //strHTML.append(temp,temp+iLength);
