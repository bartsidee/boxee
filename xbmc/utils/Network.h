...
public:
  enum EMESSAGE
  {
   ...
   virtual std::vector<CNetworkInterfacePtr> GetInterfaceList(void) = 0;
   CNetworkInterfacePtr GetInterfaceByName(const CStdString& name);

   // Return the first interface which is active
   CNetworkInterfacePtr GetFirstConnectedInterface(void);
