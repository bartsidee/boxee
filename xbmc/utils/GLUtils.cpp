...
void LogGraphicsInfo()
{
  ...
  if (s)
    CLog::Log(LOGDEBUG, "GL_VENDOR = %s", s);
  else
    CLog::Log(LOGDEBUG, "GL_VENDOR = NULL");

  s = glGetString(GL_RENDERER);
  if (s)
    CLog::Log(LOGDEBUG, "GL_RENDERER = %s", s);
  else
    CLog::Log(LOGDEBUG, "GL_RENDERER = NULL");

  s = glGetString(GL_VERSION);
  if (s)
    CLog::Log(LOGDEBUG, "GL_VERSION = %s", s);
  else
    CLog::Log(LOGDEBUG, "GL_VERSION = NULL");

  s = glGetString(GL_SHADING_LANGUAGE_VERSION);
  if (s)
    CLog::Log(LOGDEBUG, "GL_SHADING_LANGUAGE_VERSION = %s", s);
  else
    CLog::Log(LOGDEBUG, "GL_SHADING_LANGUAGE_VERSION = NULL");
...
#define GL_GPU_MEMORY_INFO_EVICTED_MEMORY_NVX            0x904B
  ...
  if (g_Windowing.IsExtSupported("GL_NVX_gpu_memory_info"))
  {
    ...
    CLog::Log(LOGDEBUG, "GL_GPU_MEMORY_INFO_TOTAL_AVAILABLE_MEMORY_NVX = %i", mem);

    //this seems to be the amount of ram on the videocard
    glGetIntegerv(GL_GPU_MEMORY_INFO_DEDICATED_VIDMEM_NVX, &mem);
    CLog::Log(LOGDEBUG, "GL_GPU_MEMORY_INFO_DEDICATED_VIDMEM_NVX = %i", mem);
  }

  s = glGetString(GL_EXTENSIONS);
  if (s)
    CLog::Log(LOGDEBUG, "GL_EXTENSIONS = %s", s);
  else
    CLog::Log(LOGDEBUG, "GL_EXTENSIONS = NULL");
