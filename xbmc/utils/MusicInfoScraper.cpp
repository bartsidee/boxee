...
void CMusicInfoScraper::FindAlbuminfo()
{
  ...
  if (scrURL.m_url.size() == 0 || !CScraperUrl::Get(scrURL.m_url[0], strHTML, m_http) || strHTML.size() == 0)
  {
    CLog::Log(LOGERROR, "%s: Unable to retrieve web site. album: <%s> artist: <%s> path: <%s>",__FUNCTION__, strAlbum.c_str(), m_strArtist.c_str(), m_info.strPath.c_str());
    return;
  }
...
bool CMusicInfoScraper::HadNetworkProblems()
{
  return m_bNetworkProblems;
}
