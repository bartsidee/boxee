...
IFileDirectory* CFactoryFileDirectory::Create(const CStdString& strPath, CFileItem* pItem, const CStdString& strMask)
{
  ...
  // minor optimization for common music files
  if (strExtension.Equals(".mp3") || strExtension.Equals(".wma") || strExtension.Equals(".flac") || strExtension.Equals(".wav") )
	  return NULL;
  ...
  if (g_advancedSettings.m_playlistAsFolders && CPlayListFactory::IsPlaylist(CFileItem(strPath,false)))
  { // Playlist file
