...
bool IDirectory::GetDirectory(const CFileItem* pItem, CFileItemList &items)
{
  bool bResult = GetDirectory(pItem->m_strPath, items);
...
void IDirectory::SetRequestAsCanceled()
{
  CSingleLock lock(m_requestCancellock);
  
  m_requestCanceled = true;
  
}

bool IDirectory::IsRequestCanceled()
{
  CSingleLock lock(m_requestCancellock);
