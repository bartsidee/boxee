...
bool CShoutcastDirectory::ParseStations(TiXmlElement *root, CFileItemList &items, CURI &url)
{
  ...
  while(element != NULL && stations < 1000)
  {
    ...
    if (!id.IsEmpty())
    {
      CFileItemPtr pItem(new CFileItem);
//Boxee
      pItem->SetProperty("shout-name", name);
      pItem->SetProperty("shout-id", id);
      pItem->SetProperty("shout-genre", genre);
//end Boxee
...
bool CShoutcastDirectory::GetDirectory(const CStdString& strPath, CFileItemList &items)
{
  ...
  char buffer[16384];
  while(!g_application.m_bStop && (size_read = http.Read(buffer, sizeof(buffer)-1)) > 0 )
  {
    buffer[size_read] = 0;
    data += buffer;
    data_size += size_read;
