...
bool CLastFMDirectory::ParseAlbumList(CStdString url, CFileItemList &items)
{
  ...
  while(pEntry)
  {
    ...
    else
    {
      ...
      TiXmlElement* pChild = pEntry->FirstChildElement("artist");
      const char *artist = ((pChild)?pChild->Attribute("name"):NULL);
...
bool CLastFMDirectory::GetDirectory(const CStdString& strPath, CFileItemList &items)
{
  ...
  {
    if(CLastFmManager::GetInstance()->LastFmHandShake() == false)
    {     
      SetRequestAsCanceled();
    }
    else
