...
CFileShoutcast::~CFileShoutcast()
{
  ...
  if (m_opt || m_pShoutCastRipper==this)
  {
    rip_manager_stop();
    delete m_opt;
    m_opt = NULL;
...
bool CFileShoutcast::Open(const CURI& url)
{
  ...
  CStdString strOperationId = "shout";
  if (dlgProgress)
  {
    ...
    if (!dlgProgress->IsDialogRunning())
      dlgProgress->StartModal(strOperationId);
    dlgProgress->Progress();
  }

  if ((ret = rip_manager_start(rip_callback, m_opt)) != SR_SUCCESS)
  {
    if (dlgProgress) CGUIDialogProgress::EndProgressOperation(strOperationId);
    return false;
  }
  ...
  while (!m_fileState.bRipDone && !m_fileState.bRipStarted && !m_fileState.bRipError && (!dlgProgress || !dlgProgress->IsCanceled()))
  {
    ...
    else
    {
      if (dlgProgress)
      {
        ...
        Sleep(1500);
        CGUIDialogProgress::EndProgressOperation(strOperationId);
      }
      return false;
    }
    iCount++;
