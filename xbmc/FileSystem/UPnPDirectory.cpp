...
{
#ifdef HAS_UPNP_AV
    CStdString strFilename, strPath = url;
...
/*----------------------------------------------------------------------
|   CUPnPDirectory::GetFriendlyPath
+---------------------------------------------------------------------*/
void CUPnPDirectory::GetFriendlyPath( CStdString& path )
{
#ifdef HAS_UPNP_AV
  DIRECTORY::CUPnPAvDirectory::GetFriendlyPath(path);
#else
  // Try to give a user friendly description of the upnp path passed in
  CUPnP* upnp = CUPnP::GetInstance();
  CStdString newPath;

  if( upnp->GetFriendlyPath( path, newPath ) )
    path = newPath;
#endif
}
...
+---------------------------------------------------------------------*/
bool CUPnPDirectory::GetResource(const CURI& path, CFileItem &item)
{
    ...
    item.SetProperty("original_listitem_mime", item.GetContentType(false));
    ...
    if (resource.m_ProtocolInfo.IsValid()) {
        ...
        if (resource.m_ProtocolInfo.GetContentType().Compare("application/octet-stream") != 0) {
            item.SetContentType((const char*)resource.m_ProtocolInfo.GetContentType());
        }
    } else {
...
CUPnPDirectory::GetDirectory(const CStdString& strPath, CFileItemList &items)
{
    ...
    if (path.CompareN( "upnp://all", 10 , true) == 0) {
    //if (path.Compare("upnp://", true) == 0) {
        upnp->StartClient();
        // root -> get list of devices, if its empty (first time launched) each item will be sent to the window lists
