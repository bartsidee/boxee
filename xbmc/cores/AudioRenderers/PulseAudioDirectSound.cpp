...
bool CPulseAudioDirectSound::Initialize(IAudioCallback* pCallback, int iChannels, unsigned int uiSamplesPerSec, unsigned int uiBitsPerSample, bool bResample, const char* strAudioCodec, bool bIsMusic, bool bPassthrough, AudioMediaFormat audioMediaFormat)
{
  ...
  if ((m_Context = pa_context_new(pa_threaded_mainloop_get_api(m_MainLoop), "Boxee")) == NULL)
  {
