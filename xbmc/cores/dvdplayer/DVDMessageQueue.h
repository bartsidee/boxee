...
public:
  ...
  void SetMaxTimeSize(double sec)       { sec == 0.0 ? m_TimeSize = 0 : m_TimeSize  = 1.0 / std::max(1.0, sec); }
  int GetMaxDataSize() const            { return m_iMaxDataSize; }
  bool IsInited() const                 { return m_bInitialized; }
  double GetFirstPTS() { return m_TimeBack; }
  double GetLastPTS() { return m_TimeFront; }
