...
bool CDVDPlayer::OpenFile(const CFileItem& file, const CPlayerOptions &options)
{
  try
  {
    ...
    while (WaitForSingleObject(m_hReadyEvent, 30) != WAIT_OBJECT_0)
    {
...
{
  return GetTotalTime() > 0 || IsPlayingStreamPlaylist();
}
...
bool CDVDPlayer::OpenVideoStream(int iStream, int source)
{
  ...
  if (m_item.IsMMS() || m_item.GetPropertyBOOL("IsLiveStream"))
    m_dvdPlayerVideo.m_messageQueue.SetMaxDataSize(2 * 256 * 1024);

  if (m_item.HasProperty("MaxVideoQueueSize"))
    m_dvdPlayerAudio.m_messageQueue.SetMaxDataSize(m_item.GetPropertyInt("MaxVideoQueueSize"));
...
bool CDVDPlayer::ChooseAndOpenSubtitleStream()
{
