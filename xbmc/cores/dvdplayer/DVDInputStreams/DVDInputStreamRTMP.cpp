...
#define GET_PROP(i,p,a) ((i).GetProperty(p).IsEmpty()?m_item.GetProperty(a):m_item.GetProperty(p))
...
bool CDVDInputStreamRTMP::Open(const char* strFile, const std::string& content)
{
  ...
  m_rtmp->SetPlayer(GET_PROP(m_item,"SWFPlayer","custom:SWFPlayer"));
  m_rtmp->SetPageUrl(GET_PROP(m_item,"PageURL","custom:PageURL"));
  m_rtmp->SetPlayPath(GET_PROP(m_item,"PlayPath","custom:PlayPath"));
  m_rtmp->SetTcUrl(GET_PROP(m_item,"TcUrl","custom:TcUrl"));
  if (m_item.HasProperty("IsLive") && m_item.GetPropertyBOOL("IsLive"))
    m_rtmp->SetLive();
  if (m_item.HasProperty("custom:IsLive") && m_item.GetPropertyBOOL("custom:IsLive"))
    m_rtmp->SetLive();
