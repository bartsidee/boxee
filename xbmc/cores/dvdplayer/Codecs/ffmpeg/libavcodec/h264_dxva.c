...
static void fill_dxva_slice_long(H264Context *h){
	...
	for(i=0; i<32; i++)
	{ pSlice->RefPicList[0][i].x.y.AssociatedFlag = 1;
	  pSlice->RefPicList[0][i].x.bPicEntry = 255; 
	  pSlice->RefPicList[0][i].x.y.Index7Bits = 127;
	  pSlice->RefPicList[1][i].x.y.AssociatedFlag = 1; 
	  pSlice->RefPicList[1][i].x.bPicEntry = 255;
	  pSlice->RefPicList[1][i].x.y.Index7Bits = 127;
	}
}
...
static int decode_nal_units_noexecute(H264Context *h, uint8_t *buf, int buf_size){
    ...
    for(;;){
        ...
        ptr= ff_h264_decode_nal_mpc(hx, buf + buf_index, &dst_length, &consumed, next_avc - buf_index);
        if (ptr==NULL || dst_length < 0){
            return -1;
        }
        while(ptr[dst_length - 1] == 0 && dst_length > 0)
            dst_length--;
        bit_length= !dst_length ? 0 : (8*dst_length - ff_h264_decode_rbsp_trailing_mpc(h, ptr + dst_length - 1));
      ...
      again:
        ...
        case NAL_PPS:
            ...
            ff_h264_decode_picture_parameter_set_mpc(h, bit_length);
