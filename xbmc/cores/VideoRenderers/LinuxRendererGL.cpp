...
void CLinuxRendererGL::RenderUpdate(bool clear, DWORD flags, DWORD alpha)
{
  ...
  if( !(m_iFlags & CONF_FLAGS_EXTERN_IMAGE) && WaitForSingleObject(m_eventTexturesDone[index], 500) == WAIT_TIMEOUT )
  {
...
void CLinuxRendererGL::AutoCrop(bool bCrop)
{
  ...
  if (bCrop)
  {
    ...
    // Crop top
    p = ( m_iFlags & CONF_FLAGS_EXTERN_IMAGE )?m_externImage:im.plane[0];
    s = p;
