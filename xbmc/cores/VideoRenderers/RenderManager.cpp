...
unsigned int CXBMCRenderManager::PreInit()
{
  if (!g_application.IsCurrentThread())
  {
    ...
    ThreadMessage tMsg = {TMSG_VIDEO_RENDERER_PREINIT};
    g_application.getApplicationMessenger().SendMessage(tMsg, true);  
    RestoreCriticalSection(g_graphicsContext, nCount);
    return tMsg.dwParam1;
  }
  return PreInit_internal();
}

unsigned int CXBMCRenderManager::PreInit_internal()
{
  CRetakeLock<CExclusiveLock> lock(m_sharedSection);
...
void CXBMCRenderManager::UnInit()
{
  ...
  {
    int nCount = ExitCriticalSection(g_graphicsContext);
    ThreadMessage tMsg = {TMSG_VIDEO_RENDERER_UNINIT};
    g_application.getApplicationMessenger().SendMessage(tMsg, true);  
    RestoreCriticalSection(g_graphicsContext, nCount);
    return;
  }
...
void CXBMCRenderManager::UnInit_internal()
{
  CRetakeLock<CExclusiveLock> lock(m_sharedSection);
