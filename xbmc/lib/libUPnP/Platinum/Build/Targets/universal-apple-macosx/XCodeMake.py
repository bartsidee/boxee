...
def usage(errMsg):
    ...
    print 'Usage: '
    print '  %s -p <path to project> -b [Release|Debug|etc.]' % (sys.argv[0])
    print ''
...
# ------------------------------------------------------------
try:
    opts, args = getopt.getopt(sys.argv[1:], "p:b:c")
...
try:
    ...
    cmd_list = ['xcodebuild', '-project', '%s' % projectFile, '-target', 'All', '-configuration', '%s' % buildName, '%s' % buildSwitch]
    cmd = " ".join(cmd_list)
    print 'Executing:'
    print cmd
