...
void vdr_dump_service_parameter_set (FILE *f,
        ...
        switch(flags->vdr_version) { // freq:<params>:source:sr:
                case 2 ... 6:
                        dump_param_vdr_1_2(f, p, flags);
                        break;
                case 7:
                default:
                        dump_param_vdr_1_7(f, p, flags);
                }
                
        fprintf (f, "%i", video_pid);

        if (video_pid && (pcr_pid != video_pid))
                fprintf (f, "+%i", pcr_pid);
        ...
        if (flags->vdr_version < 3) {
                tr_ids.original_network_id = 0;
                tr_ids.transport_stream_id = 0;
                }
