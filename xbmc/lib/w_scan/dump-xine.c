...
void xine_dump_service_parameter_set (FILE *f, 
                                ...
                                int ac3_num,
                                uint16_t *ac3_pid,
                                char ac3_lang[AUDIO_CHAN_MAX][4],
                                uint16_t service_id,
                                ...
                                struct w_scan_flags * flags)
{
        ...
        if (video_pid || audio_pid[0]) {
                ...
                {
                  if (i > 0) fprintf(f, ",");
                  fprintf (f, "%i=%s", audio_pid[i], audio_lang[i]);
                }
                fprintf (f, ":");
                for (i = 0; i < ac3_num; i++)
                {
                  if (i > 0) fprintf(f, ",");
                  fprintf (f, "%i=%s", ac3_pid[i], ac3_lang[i]);
                }
                fprintf (f, ":%i", service_id);
                fprintf (f, ":%d:%d",
                        (tr_ids.transport_stream_id > 0)?tr_ids.original_network_id:0,
                        tr_ids.transport_stream_id);
                /* what about AC3 audio here && multiple audio pids? see also: dump_mplayer.c/h */
                int minor_channel = (channel_num & 0x3FF);
                int major_channel = (channel_num >> 10) & 0x3FF;
                fprintf(f, ":%d.%d", major_channel, minor_channel);
                fprintf(f, "\n");
        }
}
