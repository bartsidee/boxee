                   ...
                   /* ITU-T Rec. H.222.0 | ISO/IEC 13818-1 PES packets containing private data*/
                        ...
                        if (find_descriptor(teletext_descriptor, buf + 5, ES_info_len, NULL, NULL)) {
                                ...
                                if (s->ac3_num < AC3_CHAN_MAX) {
                                        found=0;
                                        for (a = 0; a < s->ac3_num; a++)
                                          if (s->ac3_pid[a] == elementary_pid) found=1;
...
static void scan_tp(void)
{
        switch(fe_info.type) {
                ...
                case FE_OFDM:
                        ...
                        break;
                case FE_ATSC: {
                        int pat_timeout_retries = 10;

                        scan_tp_atsc();
                        while(pat_timeout && --pat_timeout_retries) {
                          scan_tp_atsc();
                        }
                        if(pat_timeout && !pat_timeout_retries)
                          warning("failed to read PAT\n");
        ...
        list_for_each(p1, &scanned_transponders) {
                ...
                list_for_each(p2, &t->services) {
                        ...
                        if (s->scrambled && (flags.ca_select == 0))                                     // caid, this is scrambled tv or radio
                                continue; /* FTA only */
                        /** BOXEE **/
                        /* In the boxee NYC office, we found out that on Time Warner, if there */
                        /* is CA information, it means its encrypted. Not sure if its true everywhere */
                        if (s->ca_num && (flags.ca_select == 0))
                                continue;
                        switch (output_format) {
                          case OUTPUT_VDR:
                                vdr_dump_service_parameter_set (output,
                                                   s->service_name,
                                                   s->provider_name,
                                                   &t->param,
                                                   s->video_pid,
                          ...
                          case OUTPUT_KAFFEINE:
                                kaffeine_dump_service_parameter_set (output,
                                                   s->service_name,
                                                   s->provider_name,
                                                   &t->param,
                                                   s->video_pid,
                                ...
                                break;
                          case OUTPUT_XINE:
                                xine_dump_service_parameter_set (output,
                                                   s->service_name,
                                                   s->provider_name,
                                                   &t->param,
                                                   s->video_pid,
                                ...
                                break;
                          case OUTPUT_MPLAYER:
                                mplayer_dump_service_parameter_set (output,
                                                   s->service_name,
                                                   s->provider_name,
                                                   &t->param,
                                                   s->video_pid,
                                ...
                                break;
                          case OUTPUT_VLC_M3U:
                                vlc_dump_service_parameter_set_as_xspf (output,
                                                   s->service_name,
                                                   s->provider_name,
                                                   &t->param,
                                                   s->video_pid,
...
int main (int argc, char **argv)
{
        ...
        while ((opt = getopt(argc, argv, "a:c:de:f:g:hi:kl:o:p:qr:s:t:vxA:C:D:E:FHI:LMO:PQ:R:S:T:VX")) != -1) {
                switch (opt) {
                case 'a': //adapter
                ...
                case 'g': // output to
                        ...
                        if (!output)
                        {
                          info("error creating output file: %s\n", optarg);
                          return 1;
                        }
                        break;
                case 'h': //basic help
        ...
        switch (fe_info.type) {
           ...
           case FE_ATSC:
                ...
                if (fe_info.frequency_min == 0 || fe_info.frequency_max == 0) {
                  ...
                  //fe_info.frequency_min = 369000000;
                  //fe_info.frequency_max = 369000000;
                  info("FREQ (%.2fMHz ... %.2fMHz)\n", fe_info.frequency_min/1e6, fe_info.frequency_max/1e6);
                  }
                break;
           case FE_QPSK:
        ...
        if (dry_run)
          printf("\nCOUNT: %d\n", initial_tune (frontend_fd, valid_initial_data));
        else
        {
