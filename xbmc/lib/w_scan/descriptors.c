...
void parse_iso639_language_descriptor (const unsigned char *buf, struct service *s) {
        ...
        for (i = 0; i < lang_count; i++) {
                ...
                if (s->is_last_audio_ac3)
                  memcpy(s->ac3_lang[s->ac3_num-1], buf, 3);
                else
        ...
        for (i=0; i < d.number_elements; i++) {
                ...
                switch (e.stream_type) {
                        case iso_iec_13818_1_11172_2_video_stream:
                                ...
                                break;
                        case atsc_a_52b_ac3:
                                if (s->ac3_num < AUDIO_CHAN_MAX) {
                                        found=0;
                                        for (a = 0; a < s->ac3_num; a++)
                                          if (s->ac3_pid[a] == e.elementary_PID) found=1;

                                        if (!found)
                                        {
                                          s->ac3_pid[s->ac3_num] = e.elementary_PID;
                                          s->ac3_lang[s->ac3_num][0] = (e.ISO_639_language_code >> 16) & 0xff;
                                          s->ac3_lang[s->ac3_num][1] = (e.ISO_639_language_code >> 8)  & 0xff;
                                          s->ac3_lang[s->ac3_num][2] =  e.ISO_639_language_code        & 0xff;
                                          s->ac3_num++;
                                          s->is_last_audio_ac3=1;
                                        }
                                }
                                moreverbose("\tAUDIO\t: PID 0x%04x lang: %s\n",e.elementary_PID,s->ac3_lang[s->ac3_num-1]);
