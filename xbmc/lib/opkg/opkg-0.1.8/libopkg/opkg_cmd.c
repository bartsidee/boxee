...
opkg_list_upgradable_cmd(int argc, char **argv)
{
	...
	if (_new_pkg == NULL)
        ...
        printf("pkg-name %s old-version %s  new-version %s description %s size %d installed-size %d\n",
            _old_pkg->name, old_v, new_v, _new_pkg->description, _new_pkg->size, _new_pkg->installed_size);
        free(old_v);
        free(new_v);
    }
...
static opkg_cmd_t cmds[] = {
     ...
     {"list_upgradable", 0, (opkg_cmd_fun_t)opkg_list_upgradable_cmd, PFM_SOURCE | PFM_SIZE | PFM_DESCRIPTION},
