...
void CXbmcWeb::AddItemToPlayList(const CFileItemPtr &pItem)
{
  if (pItem->m_bIsFolder)
  {
    ...
    CFileItemList items;
    DIRECTORY::CDirectory::GetDirectory(strDirectory, items);
  ...
  xbmcNavigationHandlerType	xbmcNavigator[ ] = {
    ...
    while( xbmcNavigator[ cmd].xbmcAppStateCode != -1) {
      ...
      if( stricmp( parameter,xbmcNavigator[ cmd].xbmcNavigateParameter) == 0) {
        ...
        CFileItemList libraryItemsList;
        CFileItemList internetItemsList;
        CStdString libraryDirPath;
        CStdString internetDirPath;

        //get shares and extensions
        if (!strcmp(parameter, WEB_VIDEOS))
        {
          g_playlistPlayer.SetCurrentPlaylist(PLAYLIST_VIDEO);
          libraryDirPath = "boxeedb://video";
          DIRECTORY::CDirectory::GetDirectory(libraryDirPath,libraryItemsList);
          internetDirPath = "apps://video";
          DIRECTORY::CDirectory::GetDirectory(internetDirPath,internetItemsList);
          
          m_parentFolderPath = "video";
        }
        else if (!strcmp(parameter, WEB_MUSIC))
        {
          g_playlistPlayer.SetCurrentPlaylist(PLAYLIST_MUSIC);
          libraryDirPath = "boxeedb://music";
          DIRECTORY::CDirectory::GetDirectory(libraryDirPath,libraryItemsList);
          internetDirPath = "apps://music";
          DIRECTORY::CDirectory::GetDirectory(internetDirPath,internetItemsList);
          
          m_parentFolderPath = "music";
        }
        else if (!strcmp(parameter, WEB_PICTURES))
        {
          libraryDirPath = "boxeedb://pictures";
          DIRECTORY::CDirectory::GetDirectory(libraryDirPath,libraryItemsList);
          internetDirPath = "apps://pictures";
          DIRECTORY::CDirectory::GetDirectory(internetDirPath,internetItemsList);
          
          m_parentFolderPath = "pictures";
        }
        /*
        ...
        */
        ...
        CFileItemList fixInternetItemsList;
        for(int i=0;i<internetItemsList.Size();i++)
        {
          CFileItemPtr item = internetItemsList.Get(i);

          if((item->m_strPath).Left(9) != "plugin://")
          {
            fixInternetItemsList.Add(item);
          }
        }

        webDirItems->Append(libraryItemsList);
        webDirItems->Append(fixInternetItemsList);
        
        //sort items
        webDirItems->Sort(SORT_METHOD_LABEL, SORT_ORDER_ASC);
        
        m_dirHistoryPaths.clear();
        m_parentDirHistoryPaths.clear();
        ...
        else
        {
          ...
          if (itm->m_bIsFolder)
          {
            ...
            if (itm->GetLabel() == "..")
            {
              ...
              // Get the directory from history
              if(!m_dirHistoryPaths.empty())
              {
                *webDirItems = m_dirHistoryPaths.back();
                m_dirHistoryPaths.pop_back();
                
                //set new current directory for webserver
                CStdString strParentPath = m_parentDirHistoryPaths.back();
                m_parentDirHistoryPaths.pop_back();
                
                if(m_parentDirHistoryPaths.size() < 2)
                {
                  m_parentFolderPath = strParentPath;
                }
                
                SetCurrentDir(strParentPath.c_str());
              }
              else
              {
            ...
            else
            {
              ...
              CStdString strParentPath = m_parentFolderPath;
              
              // Add current dir to history and save this path as the parent of the next path
              m_dirHistoryPaths.push_back(*webDirItems);
              m_parentDirHistoryPaths.push_back(m_parentFolderPath);
              ...
              DIRECTORY::CDirectory::GetDirectory(strDirectory, *webDirItems);

              webDirItems->Sort(SORT_METHOD_LABEL, SORT_ORDER_ASC);
              
              m_parentFolderPath = strDirectory;
            }
          }
          else
          {
...
int CXbmcWeb::xbmcCommand( int eid, webs_t wp, int argc, char_t **argv)
{
  ...
  if(g_application.IsUserLoggedIn() == false)
  {
    websError(wp, 500, T((char*)"No user logged in\n"));
    return -1;
  }
