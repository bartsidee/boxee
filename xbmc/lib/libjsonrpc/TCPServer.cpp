...
void CTCPServer::Announce(ANNOUNCEMENT::EAnnouncementFlag flag, const char *sender, const char *message, const CVariant &data)
{
  Value root;
  root["jsonrpc"] = "2.0";
...
bool CTCPServer::Initialize()
{
  ...
  int rc = 1;
  setsockopt(m_ServerSocket, SOL_SOCKET, SO_REUSEADDR, (char *)&rc, sizeof(rc));
...
{
  m_announcementflags = ANNOUNCEMENT::AF_None;
  m_socket = -1;
  m_beginBrackets = 0;
  m_endBrackets = 0;
  m_permissions = None;
...
CTCPServer::CTCPClient::CTCPClient(const CTCPClient& client) : IClient(client)
{
  Copy(client);
}
...
void CTCPServer::CTCPClient::Copy(const CTCPClient& client)
{
  ...
  m_permissions       = client.m_permissions;
}
