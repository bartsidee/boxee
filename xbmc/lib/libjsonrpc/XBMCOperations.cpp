...
JSON_STATUS CXBMCOperations::Play(const CStdString &method, ITransportLayer *transport, IClient *client, const Value &parameterObject, Value &result)
{
  ...
  CFileItem playItem(file);
  playItem.m_strPath = file;
  playItem.SetProperty("url_source", "other"); // set value to "browser-app", if you don't want our js to load

  if (parameterObject.isMember("contenttype"))
  {
    if (!parameterObject["contenttype"].isString())
      return InvalidParams;

    playItem.SetContentType(parameterObject["contenttype"].asString());
  }
  ...
  g_application.getApplicationMessenger().MediaPlay(playItem);
