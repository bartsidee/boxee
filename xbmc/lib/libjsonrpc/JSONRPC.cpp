...
// Files
  ...
  { "Files.DeleteSource",                           CFileOperations::DeleteSource },
...
// XBMC operations
  ...
  { "Device.PairChallenge",                         CDeviceOperations::PairChallenge },
  { "Device.PairResponse",                          CDeviceOperations::PairResponse },
  { "Device.Unpair",                                CDeviceOperations::Unpair },
  { "Device.Connect",                               CDeviceOperations::Connect },

  { "GUI.TextFieldSet",                             CGUIOperations::TextFieldSet },
  { "GUI.KeyboardState",                            CGUIOperations::KeyboardState },
  { "GUI.NotificationShow",                         CGUIOperations::NotificationShow },
  ...
  { "Input.MouseMovement",                          CInputOperations::MouseMovement },
  { "Input.MouseClick",                             CInputOperations::MouseClick },
  { "Input.NavigationState",                        CInputOperations::NavigationState },
};
...
JSON_STATUS CJSONRPC::GetNotificationFlags(const CStdString &method, ITransportLayer *transport, IClient *client, const Json::Value& parameterObject, Json::Value &result)
{
  ...
  for (int i = 1; i <= ANNOUNCE_ALL; i *= 2)
    result[NotificationFlagToString((ANNOUNCEMENT::EAnnouncementFlag)i)] = (flags & i) > 0;
...
JSON_STATUS CJSONRPC::SetNotificationFlags(const CStdString &method, ITransportLayer *transport, IClient *client, const Json::Value& parameterObject, Json::Value &result)
{
  ...
  if (parameterObject["Playback"].asBool())
    flags |= ANNOUNCEMENT::AF_Playback;
  if (parameterObject["GUI"].asBool())
    flags |= ANNOUNCEMENT::AF_GUI;
  if (parameterObject["System"].asBool())
    flags |= ANNOUNCEMENT::AF_System;
  if (parameterObject["Library"].asBool())
    flags |= ANNOUNCEMENT::AF_Library;
  if (parameterObject["Other"].asBool())
    flags |= ANNOUNCEMENT::AF_Other;
...
JSON_STATUS CJSONRPC::Notify(const CStdString &method, ITransportLayer *transport, IClient *client, const Json::Value& parameterObject, Json::Value &result)
{
  if (parameterObject["data"].isNull())
    ANNOUNCEMENT::CAnnouncementManager::Announce(ANNOUNCEMENT::AF_Other, parameterObject["sender"].asString().c_str(),
