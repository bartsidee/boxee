...
//Boxee
  PyDoc_STRVAR(getFileHash__doc__,
    "getFileHash(path) -- Returns the hash for the specified filename.\n"
    "\n"
    "path           : string or unicode - path to file\n"
    "\n"
    "example:\n"
    "  - hash = xbmc.getFileHash('f:\\\\videos\\\\movie.avi')\n");
  ...
  PyObject* XBMC_GetFileHash(PyObject *self, PyObject *args)
  {
    ...
    CLog::Log(LOGDEBUG,"Calculating hash for <%s>", strText.c_str());
    
#ifdef _WIN32
    uint64_t hash = CMetadataResolverVideo::CalculateVideoHash(strText);
#else
    __uint64 hash = CMetadataResolverVideo::CalculateVideoHash(strText);
#endif
    char strHash[17];
    memset(strHash,0,17);
  ...
  PyDoc_STRVAR(getFileSize__doc__,
    "getFileSize(path) -- Returns the size for the specified filename (in bytes).\n"
    "\n"
    "path           : string or unicode - path to file\n"
    "\n"
    "example:\n"
    "  - size = xbmc.getFileSize('f:\\\\videos\\\\movie.avi')\n");
  ...
  PyDoc_STRVAR(getBoxeeCookieJar__doc__,
               "getBoxeeCookieJar() -- Returns the boxee cookiejar file.\n"
               "\n"
               "example:\n"
               "  - size = xbmc.getBoxeeCookieJar()\n");
  ...
  PyDoc_STRVAR(showNotification__doc__,
               "showNotification(caption, description, icon, duration) \n"
               "\n"
               "caption           : string or unicode \n"
               "description       : string or unicode \n"
               "icon              : string or unicode - path to icon file\n"
               "duration          : int (optional) = TOAST_DISPLAY_TIME - seconds to show\n"
               "\n"
               "example:\n"
               "  - size = xbmc.showNotification('title','message','f:\\\\videos\\\\cute.tbn')\n");
  
  PyObject* XBMC_ShowNotification(PyObject *self, PyObject *args)
  {
    char *cCaption = NULL;
    char *cDesc = NULL;
    char *cIcon = NULL;
    int duration = TOAST_DISPLAY_TIME;
    CStdString ret;
    if (PyArg_ParseTuple(args, (char*)"ss|s|i", &cCaption, &cDesc, &cIcon, &duration))
    {
      CStdString strCaption, strDesc, strIcon;
      if (cCaption)
        strCaption = cCaption;
      if (cDesc)
        strDesc = cDesc;
      ...
      g_application.m_guiDialogKaiToast.QueueNotification(strIcon, strCaption, strDesc, duration);
    }
  ...
  PyDoc_STRVAR(openBrowse__doc__,
               "openBrowse(path) -- open browse screen on a path.\n"
               "\n"
               "path           : string or unicode - path to open\n"
               "\n"
               "example:\n"
               "  - size = xbmc.openBrowse('f:\\\\videos')\n");
  ...
  PyDoc_STRVAR(noCachePath__doc__,
               "noCachePath(path) -- add the path to the list of paths no being cached by directory cache.\n"
               "\n"
               "path           : string or unicode - path to add\n"
               "\n"
               "example:\n"
               "  - size = xbmc.noCachePath('plugin:\\\\youtbe\\search')\n");
  ...
  PyObject* XBMC_NoCachePath(PyObject *self, PyObject *args)
  {
    ...
    g_directoryCache.AddDirectoryToBlacklist(strText);
  ...
  PyMethodDef xbmcMethods[] = {
    ...
    {(char*)"getBoxeeCookieJar", (PyCFunction)XBMC_GetBoxeeCookieJar, METH_VARARGS, getBoxeeCookieJar__doc__},
    {(char*)"getFileHash", (PyCFunction)XBMC_GetFileHash, METH_VARARGS, getFileHash__doc__},
    {(char*)"getFileSize", (PyCFunction)XBMC_GetFileSize, METH_VARARGS, getFileSize__doc__},
    {(char*)"showNotification", (PyCFunction)XBMC_ShowNotification, METH_VARARGS, showNotification__doc__},
    {(char*)"openBrowse", (PyCFunction)XBMC_OpenBrowse, METH_VARARGS, openBrowse__doc__},
    {(char*)"noCachePath", (PyCFunction)XBMC_NoCachePath, METH_VARARGS, noCachePath__doc__},
