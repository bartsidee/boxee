...
namespace PYXBMC
{
  ...
  PyObject* ControlLabel_SetLabel(ControlLabel *self, PyObject *args)
  {
    ...
    msg.dwParam2 = pControl->iControlId;
    msg.lpVoid = pControl->pGUIControl;
    msg.strParam = self->strText;
    g_application.getApplicationMessenger().SendMessage(msg,false); // dont lock. it causes a dead lock and i am not yet sure why (graphic context).
