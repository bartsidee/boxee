...
PyObject *pending_threadfunc(PyObject *self, PyObject *arg)
{
	...
	int n;
	int i, j, k;
	if (PyArg_ParseTuple(arg, "Oi", &callable, &n) == 0)
		return NULL;

	/* create references upfront, while we hold the lock */
	for(i = 0; i<n; i++)
		Py_INCREF(callable);
	...
	k = 0;
	for(i = 0; i<n; i++) {
		int r = Py_AddPendingCall(&_pending_callback, callable);
		if (r < 0)
			i--; /* deal with full queue */
		if (i+1<n) {
			/* sleep a bit.  No good portable way to do this, so lets just spend
			 * cpu cycles
			 */
