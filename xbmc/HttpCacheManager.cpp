...
void CHttpCacheManager::UpdateRefMap(const std::string  &strUrl, unsigned long  accessTime, int  refDiff, bool  progressStat, bool shouldUpdateProgress)
{
  ...
  if (objRef)
  {
	...
	{
	  CLog::Log(LOGWARNING,"CHttpCacheManager::UpdateRefMap negative reference count for url %s (hcm)", strUrl.c_str());
	  return;
	}
  ...
  else
  {
    ...
    if ((refDiff < 0 ))
    {
	  CLog::Log(LOGDEBUG,"CHttpCacheManager::UpdateRefMap Can not decrease ref count to new object %s (hcm)", strUrl.c_str());
	  return;
    }
	else
	{
  ...
  CLog::Log(LOGDEBUG,"update ref map. url %s new ref_count %ld  progress %d (hcm)", strUrl.c_str(),
		  m_refMap[strUrl]->m_refCount, m_refMap[strUrl]->m_progress);


}
bool CHttpCacheManager::IsUrlInMap(const std::string  &strUrl)
{
  return  (m_refMap[strUrl] != NULL);
}
...
void CHttpCacheManager::RemoveFromRefMap(const std::string  &strUrl)
{
  if (m_refMap[strUrl])
  {
    CLog::Log(LOGDEBUG,"CHttpCacheManager::RemoveFromRefMap remove url %s  (hcm)", strUrl.c_str());
	CacheRefObj *objRef = m_refMap[strUrl];
...
bool CHttpCacheManager::Initialize(const std::string &strLocalCachePath)
{
  ...
  else
  {
    ...
    if ( pDS->fv(0).get_asInt() < HTTP_CACHE_DB_VERSION )
    {
      ...
      DBAction(pDatabase, false, "update tb_cache_map set access_time = -1,  ref_count  = -1, in_progress = -1");

      DBAction(pDatabase, false, "delete from tb_version\n");
      DBAction(pDatabase, false, "insert into tb_version (vernum) values (%d)", HTTP_CACHE_DB_VERSION);
    }
  }
 ...
 // there is no point to check if the file's status since we add it to the database only after we done te process
  if (pDS && pDS->num_rows() > 0 )
  {
    ...
    if (!pDS->fv(3).get_isNull())
      httpTime = curl_getdate(pDS->fv(3).get_asString().c_str(), 0);
...
void CHttpCacheManager::DoneCachingURL(HttpCacheHandle h, const std::string &strUrl, long nHttpRespCode, const HttpCacheHeaders & cacheHeaders)
{
  ...
  if (bShouldCache)
  {
    ...
    {
      tmServer = (unsigned int)curl_getdate(date->second.c_str(),NULL);
    }
    ...
    {
      unsigned int exp = (unsigned int)curl_getdate(expiry->second.c_str(), NULL);
      if (exp < tmServer)
        tmExpire = 0;
      else
...
{
  char *e = curl_escape(strUrl.c_str(), strUrl.size());
  if (e)
  {
