...
bool CPicture::CreateThumbnail(const CStdString& file, const CStdString& thumbFile, bool checkExistence /*= false*/)
{
  ...
  {
    CLog::Log(LOGDEBUG, "%s: Unable to create thumb from [image=%s] because thumbfile is EMPTY [thumbfile=%s]", __FUNCTION__, file.c_str(), thumbFile.c_str());
    return false;
  }
...
{
  CLog::Log(LOGDEBUG, "Caching image from: %s to %s", sourceFile.c_str(), destFile.c_str());
...
{
  CLog::Log(LOGDEBUG, "Creating album thumb from memory: %s", thumbFile.c_str());
  DllImageLib dll;
  if (!dll.Load()) return false;
