...
#define KILL_XBMC_BUTTON "39_"

#define LOG if (m_bVerbose) printf
#define APPLICATION_NAME "Boxee"

#define SERVER_PORT 9770
...
{
    std::string    *key = key_cookies10_5;
    std::string     prefix;
...
bool AppleRemote::SendCommand(const std::string &key)
{
	...
	if (pPacket)
	{
		LOG("Sending command for Key (%s) to <%s>\n", key.c_str(), m_serverAddress.c_str());
		CAddress addr(m_serverAddress.c_str(), m_port);
...
{
        LOG("Trying to start BOXEE.\n");

        int      result = -1;
        uint32_t path_size = 2*MAXPATHLEN;
        char     given_path[path_size];
        memset(given_path, 0, path_size);
        result = _NSGetExecutablePath(given_path, &path_size);
        if (result == 0)
        {
                std::string strPath = given_path;
                size_t pos = strPath.find(".app");
                if (pos != std::string::npos)
                {
                        std::string strCmd = "/usr/bin/open "+strPath.substr(0,pos+4);
...
void AppleRemote::OnKeyDown(const std::string &key)
{
	if (!IsProgramRunning(APPLICATION_NAME) && (m_serverAddress == "localhost" || m_serverAddress == "127.0.0.1"))
	{
        ...
        if (key == KILL_XBMC_BUTTON)
        {
                system("killall -9 "APPLICATION_NAME);
                return;
        }
...
void AppleRemote::SetPort(short nPort)
{
    m_port = nPort;
}
