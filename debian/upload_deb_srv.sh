...
rm -rf tmp
mkdir -p tmp/dists/${ubuntu}/main/binary-i386
cd tmp
echo "Downloading Packages.gz"
wget -q http://${http_path}/dists/${ubuntu}/main/binary-i386/Packages.gz
if [ "$?" != "0" ]; then
  echo "Could not download Packages.gz"
  exit 1
fi
gunzip Packages.gz
cp ../$1 dists/${ubuntu}/main/binary-i386
...
scp ../$1 ${up_user}@${scp_path}/dists/${ubuntu}/main/binary-i386

echo "Uploading Packages.gz, please provide password"
scp Packages.gz ${up_user}@${scp_path}/dists/${ubuntu}/main/binary-i386
