    ...
    def _get_files( self, script_files, version ):
        ...
        try:
            for cnt, url in enumerate( script_files ):
                ...
                path = items[ 0 ].replace( "/tags/%s/" % ( __scriptname__.replace( " ", "%20" ), ), "special://xbmc/scripts/%s_v" % ( __scriptname__, ) ).replace( "%20", " " )
            ...
            self.dialog.close()
            xbmcgui.Dialog().ok( __scriptname__, _( 1010 ), "special://xbmc/scripts/%s_v%s/" % ( __scriptname__, version, ) )
