...
void CGUIListItem::Archive(CArchive &ar)
{
  if (ar.IsStoring())
  {
    ...
    // counting on char being single byte. 
    char nMark = '*';
    ar << nMark;
...
const CStdString &CGUIListItem::GetProperty(const CStdString &strKey) const
{
  CSingleLock lock(m_lock);
...
{
  const CStdString &strProp = GetProperty(strKey);
  return strProp == "1" || strProp == "true";
}
