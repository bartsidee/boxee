...
#ifdef APP_JS  
  if (langStr.CompareNoCase("javascript") == 0 || langStr.CompareNoCase("js") == 0)
    action.m_lang = CGUIActionDescriptor::LANG_JAVASCRIPT;
  else
#endif
...
bool CGUIControlFactory::GetTexture(const TiXmlNode* pRootNode, const char* strTag, CTextureInfo &image)
{
  ...
  const char* platformClass = pNode->Attribute("platformClass");
  if (platformClass)
  {
#ifdef HAS_EMBEDDED
    if (strcmpi(platformClass, "pc") == 0) return false;
#endif
#ifndef HAS_EMBEDDED
    if (strcmpi(platformClass, "emb") == 0) return false;
#endif
  }
  ...
  if (background && strnicmp(background, "true", 4) == 0)
    image.useLarge = true;
  const char *blend = pNode->Attribute("blend");
  if (blend && strcmpi(blend, "false") == 0) image.blendCenter = false;
  const char *blendBorder = pNode->Attribute("blendborder");
  if (blendBorder && strcmpi(blendBorder, "false") == 0) image.blendBorder = false;
...
CGUIControl* CGUIControlFactory::Create(int parentID, const CRect &rect, TiXmlElement* pControlNode, bool insideContainer)
{
  ...
  bool bScrollLabelOnFocus = false;
  bool bPulse = true;
  ...
  XMLUtils::GetBoolean(pControlNode, "canfocus", canFocus);
  ...
  GetTexture(pControlNode, "textureclick", textureClick);  
  ...
  XMLUtils::GetBoolean(pControlNode, "scrollonfocus", bScrollLabelOnFocus);
  ...
  if (XMLUtils::GetString(pControlNode, "selectionmode", selectionLabel))
  {
    if (selectionLabel.CompareNoCase("single") == 0)
    {
      selectionMode = SELECTION_SINGLE;
    }
    else if (selectionLabel.CompareNoCase("multi") == 0)
    {
  ...
  else if (strType == "label")
  {
    ...
    { // inside lists we use CGUIListLabel
      ...
      ((CGUIListLabel *) control)->SetWrap(wrapMultiLine);
      ((CGUIListLabel *) control)->SetScrollOnFocus(bScrollLabelOnFocus);
    }
    else
    {
  ...
  {
    control = new CGUIKeyboardControl(
        parentID, id, posX, posY, width, height, textureFocus, textureNoFocus,
        labelInfo, strLabel);

    if (bPassword)
      ((CGUIKeyboardControl *) control)->SetInputType(CGUIEditControl::INPUT_TYPE_PASSWORD, 0);
    ((CGUIKeyboardControl *) control)->SetTextChangeActions(textChangeActions);
  }
  ...
  else if (strType == "button")
  {
    ...
    {
      ((CGUIButtonControl *)control)->SetClickTexture(textureClick, clickDuration);
    }
  }
  ...
  else if (strType == "list")
  {
    ...
    ((CGUIListContainer *)control)->SetAutoScrollTime(autoScrollTime);
    ((CGUIListContainer *)control)->SetOffsets(offsetX, offsetY);
  }
  else if (strType == "wraplist")
  {
    control = new CGUIWrappingListContainer(parentID, id, posX, posY, width, height, orientation, scrollTime, preloadItems, focusPosition, selectionMode);
    ...
    ((CGUIWrappingListContainer *)control)->SetAutoScrollTime(autoScrollTime);
    ((CGUIWrappingListContainer *)control)->SetOffsets(offsetX, offsetY);
  }
  else if (strType == "fixedlist")
  {
    ...
    ((CGUIFixedListContainer *)control)->SetAutoScrollTime(autoScrollTime);
    ((CGUIFixedListContainer *)control)->SetOffsets(offsetX, offsetY);
    ((CGUIFixedListContainer *)control)->PinPosition(bPinnedPosition);
  }
  else if (strType == "panel")
  {
    ...
    ((CGUIPanelContainer *)control)->SetAutoScrollTime(autoScrollTime);
    ((CGUIPanelContainer *)control)->SetOffsets(offsetX, offsetY);
  }
