...
CAnimEffect::CAnimEffect(const TiXmlElement *node, EFFECT_TYPE effect)
{
  m_effect = effect;
  // defaults
  m_delay = m_length = m_deltax = m_deltay = 0;
  m_pTweener = NULL;
  // time and delay
  ...
  if (g_SkinInfo.ResolveConstant(node->Attribute("deltax"), temp)) m_deltax = (unsigned int)(temp * g_SkinInfo.GetEffectsSlowdown());
  if (g_SkinInfo.ResolveConstant(node->Attribute("deltay"), temp)) m_deltay = (unsigned int)(temp * g_SkinInfo.GetEffectsSlowdown());
...
const CAnimEffect &CAnimEffect::operator=(const CAnimEffect &src)
{
  ...
  m_delay = src.m_delay;
  m_deltax = src.m_deltax;
  m_deltay = src.m_deltay;
...
void CAnimation::Create(const TiXmlElement *node, const CRect &rect)
{
  ...
  else if (type.Equals("click"))  m_type = ANIM_TYPE_CLICK;
  else if (type.Equals("listload"))  m_type = ANIM_TYPE_LIST_LOAD;
  // sanity check
...
void CAnimation::UpdateDelay(int row, int col)
{
  for (unsigned int i = 0; i < m_effects.size(); i++)
  {
    CAnimEffect* effect = m_effects[i];    
    unsigned int delay = 0;//effect->GetDelay();
    delay += (row * effect->GetDeltaX()) + (col * effect->GetDeltaY()); 
