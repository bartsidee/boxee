...
CGUIListLabel::CGUIListLabel(int parentID, int controlID, float posX, float posY, float width, float height, const CLabelInfo& labelInfo, const CGUIInfoLabel &info, bool alwaysScroll, int scrollSpeed)
    ...
    , m_textLayout(labelInfo.font, false, height)
    , m_scrollInfo(50, 0, scrollSpeed)
    , m_renderRect()
{
  m_selected = false;
  m_bWrap = false;
  m_scrollOnFocus = false;
...
{
  if (m_alwaysScroll || (m_scrollOnFocus && scrolling))
    m_scrolling = true;
  else
    m_scrolling = false;
...
void CGUIListLabel::SetScrollOnFocus(bool bScrollOnfocus)
{
  m_scrollOnFocus = bScrollOnfocus;
}

void CGUIListLabel::SetWrap(bool bWrap)
{
  m_bWrap = bWrap;
  m_textLayout.SetWrap(m_bWrap);
}
...
void CGUIListLabel::Render()
{
  ...
  else
  {
    ...
    uint32_t align = m_scrolling?0:XBFONT_TRUNCATED; // always truncate strings
    if (!needsToScroll)
...
{
  if (m_textLayout.Update(label, GetWidth(), m_bInvalidated))
