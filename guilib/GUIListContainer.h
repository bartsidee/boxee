...
{
public:
  CGUIListContainer(int dwParentID, int dwControlId, float posX, float posY, float width, float height, ORIENTATION orientation, int scrollTime, int preloadItems, ListSelectionMode selectionMode);
//#ifdef PRE_SKIN_VERSION_9_10_COMPATIBILITY
  CGUIListContainer(int parentID, int controlID, float posX, float posY, float width, float height,
                         ...
                         float textureHeight, float itemWidth, float itemHeight, float spaceBetweenItems, ListSelectionMode selectionMode);
//#endif
