  ...
  if (m_keyEvent.type == SDL_KEYDOWN)
  {
    ...
    else
    {
      ...
      if (!m_VKey && !m_cAscii) // split block due to ms compiler complaints about nested code depth
      {
        ...
        if (m_keyEvent.key.keysym.sym == SDLK_BACKSPACE) { m_VKey = 0x08; m_wUnicode=0; }   
        else if (m_keyEvent.key.keysym.sym == SDLK_TAB) m_VKey = 0x09;
        else if (m_keyEvent.key.keysym.sym == SDLK_RETURN) { m_VKey = 0x0d; m_wUnicode=0; }
        ...
        else if (m_keyEvent.key.keysym.sym == SDLK_KP_ENTER) { m_VKey = 0x6C; m_wUnicode=0; }
