...
void CGUIListGroup::AddAnimation(const CAnimation &animation)
{
  CGUIControl::AddAnimation(animation);
}

bool CGUIListGroup::IsAnimating(ANIMATION_TYPE anim)
{
  if (CGUIControl::IsAnimating(anim)) return true;
  
  for (iControls it = m_children.begin(); it != m_children.end(); ++it)
  {
    if ((*it)->IsAnimating(anim))
    {
      return true;
    }
  }
