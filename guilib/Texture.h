...
public:
  CBaseTexture(unsigned int width = 0, unsigned int height = 0, unsigned int format = XB_FMT_B8G8R8A8);
  virtual ~CBaseTexture();

  bool LoadFromFile(const CStdString& texturePath, unsigned int maxHeight = 0, unsigned int maxWidth = 0,
                    bool autoRotate = false, unsigned int *originalWidth = NULL, unsigned int *originalHeight = NULL, unsigned int dstWidth = 0, unsigned int dstHeight = 0);
  bool LoadFromMemory(unsigned int width, unsigned int initialWidth, unsigned int height, unsigned int initialHeight, unsigned int pitch, unsigned int format, unsigned char* pixels);
  ...
  bool LoadFromTexture(unsigned int width, unsigned int height, unsigned int format, XBMC::TexturePtr texture);

  bool IsTextureOwner() { return m_textureOwner; }
...
protected:
  ...
  //for 'twiddled' images (pvr texture_
  unsigned int m_imageInitialWidth;
  unsigned int m_imageInitialHeight;
#ifdef HAS_DX
