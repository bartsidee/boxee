...
CGUIWrappingListContainer::CGUIWrappingListContainer(int parentID, int controlId, float posX, float posY, float width, float height, ORIENTATION orientation, int scrollTime, int preloadItems, int fixedPosition, ListSelectionMode selectionMode)
...
bool CGUIWrappingListContainer::SelectItemFromPoint(const CPoint &point)
{
  ...
  { // scroll backward
    ...
    CLog::Log(LOGDEBUG, "%s: Speed %f", __FUNCTION__, amount);
