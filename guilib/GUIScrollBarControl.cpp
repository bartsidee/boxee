...
bool CGUIScrollBar::OnAction(const CAction &action)
{
  ...
  case ACTION_MOVE_LEFT:
    if (m_orientation == HORIZONTAL)
    {
      if (m_offset == 0)
      {
        // at the most left of scroll -> in case of OnLeft -> move to it

        CGUIMessage msg(GUI_MSG_SETFOCUS, GetID(), GetControlIdLeft(), 0);
  ...
  case ACTION_MOVE_RIGHT:
    if (m_orientation == HORIZONTAL)
    {
      if (m_offset + m_pageSize >= m_numItems)
      {
        // at the most right of scroll -> in case of OnRight -> move to it

        CGUIMessage msg(GUI_MSG_SETFOCUS, GetID(), GetControlIdRight(), 0);
        SendWindowMessage(msg);
      }
      else
      {
  ...
  case ACTION_MOVE_UP:
    if (m_orientation == VERTICAL)
    {
      if (m_offset == 0)
      {
        // at the most up of scroll -> in case of OnUp -> move to it

        CGUIMessage msg(GUI_MSG_SETFOCUS, GetID(), GetControlIdUp(), 0);
        SendWindowMessage(msg);
      }
      else
      {
  ...
  case ACTION_MOVE_DOWN:
    if (m_orientation == VERTICAL)
    {
      if (m_offset + m_pageSize >= m_numItems)
      {
        // at the most down of scroll -> in case of OnDown -> move to it

        CGUIMessage msg(GUI_MSG_SETFOCUS, GetID(), GetControlIdDown(), 0);
        SendWindowMessage(msg);
      }
      else
      {
