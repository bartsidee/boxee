...
CGUILabelControl::CGUILabelControl(int dwParentID, int dwControlId, float posX, float posY, float width, float height, const CLabelInfo& labelInfo, bool wrapMultiLine, bool bHasPath)
    : CGUIControl(dwParentID, dwControlId, posX, posY, width, height), m_textLayout(labelInfo.font, wrapMultiLine, height)
{
  m_bHasPath = bHasPath;
  m_iCursorPos = 0; 
  m_label = labelInfo;
...
CStdString CGUILabelControl::GetLabel() const
{
...
bool CGUILabelControl::OnMessage(CGUIMessage& message)
{
  if ( message.GetControlId() == GetID() )
  {
    ...
    else if (message.GetMessage() == GUI_MSG_GET_LABEL)
    {
