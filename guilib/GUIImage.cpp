    ...
    : CGUIControl(left), m_texture(left.m_texture)
{
  ...
  m_currentFadeTime = 0;
  m_lastRenderTime = 0;
  m_renderBorderOnly = left.m_renderBorderOnly;
  ControlType = GUICONTROL_IMAGE;
  m_bDynamicResourceAlloc=false;
}
...
void CGUIImage::Render()
{
  ...
  m_texture.SetRenderBorderOnly(m_renderBorderOnly);
...
bool CGUIImage::OnMessage(CGUIMessage& message)
{
  ...
  {
    SetFileName(message.GetStringParam());
    return true;
  }
...
void CGUIImage::SetRenderBorderOnly(bool renderBorderOnly)
{
