...
bool CSurface::ResizeSurface(int newWidth, int newHeight)
{
  ...
  if (SDL_GetWMInfo(&sysInfo))
  {
    ...
    if(m_iMonitorID == -1)
      m_iMonitorID = CWin32MonitorsConfig::GetPrimaryMonitorId();

    MONITOR_DETAILS monitorDetails;
    CWin32MonitorsConfig::GetMonitorConfig(m_iMonitorID, &monitorDetails);
    hMonitor = monitorDetails.hMonitor;
    ...
    // This will fix the bug with the fullscreen->window mode.
    // However it will reset all the graphics, so we dont use for now
    /*
    if(!m_bCoversScreen)
    {
      SDL_SetVideoMode(newWidth, newHeight, 0, SDL_RESIZABLE | SDL_OPENGL);
      m_glDC = wglGetCurrentDC();
      m_glContext = wglGetCurrentContext();
