  ...
  case GUI_MSG_SETFOCUS:
    {
      ...
      if (!m_defaultAlways && m_focusedControl)
      {
        ...
        if (control)
        {
          CGUIMessage msg(GUI_MSG_SETFOCUS, GetParentID(), control->GetID(), message.GetParam1());
...
void CGUIControlGroup::SetCanFocus(bool canFocus)
{
  CGUIControl::SetCanFocus(canFocus);
  for (ciControls it = m_children.begin(); it != m_children.end(); ++it)
  {
    (*it)->SetCanFocus(canFocus);
  }
}
...
void CGUIControlGroup::QueueAnimation(ANIMATION_TYPE animType)
{
  ...
  if (animType == ANIM_TYPE_WINDOW_OPEN || animType == ANIM_TYPE_WINDOW_CLOSE || animType == ANIM_TYPE_CLICK || animType == ANIM_TYPE_LIST_LOAD)
  {
    for (iControls it = m_children.begin(); it != m_children.end(); ++it)
