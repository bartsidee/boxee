...
bool CGUIWindow::Load(const CStdString& strFileName, bool bContainsPath)
{
  ...
  {
    // BOXEE 
    // this was done so the apps will not be rendered in a small area which in full screen
    resToUse = RES_HDTV_720p;
    // END BOXEE
    strPath = strFileName;
  }
  else 
...
bool CGUIWindow::Load(TiXmlDocument &xmlDoc)
{
  ...
  TiXmlElement* pChild = pRootElement->FirstChildElement("ontimer");
  while (pChild)
  {
    CWindowTimer windowTimer;
    
    if (CGUIControlFactory::GetAction(pChild, windowTimer.actions))
    {
      if (pChild->Attribute("interval") && atol(pChild->Attribute("interval")) > 0)
      {
        windowTimer.interval = atol(pChild->Attribute("interval"));
        m_timerActions.push_back(windowTimer);
      }
    }    
    pChild = pChild->NextSiblingElement("ontimer");
  }
  
  pChild = pRootElement->FirstChildElement();
  while (pChild)
  {
...
void CGUIWindow::LoadControl(TiXmlElement* pControl, CGUIControlGroup *pGroup)
{
  ...
  const char* platformClass = pControl->Attribute("platformClass");
  if (platformClass)
  {
#ifdef HAS_EMBEDDED
    if (strcmpi(platformClass, "pc") == 0) ignoreControl = true;
#endif
#ifndef HAS_EMBEDDED
    if (strcmpi(platformClass, "emb") == 0) ignoreControl = true;
#endif
  }
...
bool CGUIWindow::OnMouse(const CPoint &point)
{
  ...
  { // no control found to absorb this click - go to previous menu
    g_Mouse.bClick[MOUSE_RIGHT_BUTTON] = false; // must reset the right click so it wont propegate further
    CAction action;
...
bool CGUIWindow::HandleMouse(CGUIControl *pControl, const CPoint &point)
{
  ...
  { // Left click
    g_Mouse.bClick[MOUSE_LEFT_BUTTON] = false; // must reset the left click so it wont propegate further
...
void CGUIWindow::OnInitWindow()
{
  ...
  // Reset the timers
  if (m_timerActions.size() > 0)
  {
    for (size_t i = 0; i < m_timerActions.size(); i++)
    {
      m_timerActions[i].timer.StartZero();
    }
  }
...
void CGUIWindow::OnDeinitWindow(int nextWindowID)
{
  ...
  if (nextWindowID != WINDOW_FULLSCREEN_VIDEO)
  {
    ...
    if (HasAnimation(ANIM_TYPE_WINDOW_CLOSE) && !IsDialog() && IsActive() && !g_application.m_bStop)
    {
  ...
  g_windowManager.SetPreviousWindow(GetID());
}
...
bool CGUIWindow::OnMessage(CGUIMessage& message)
{
  ...
  case GUI_MSG_MOVE:
    {
      if (HasID(message.GetSenderId()))
        return OnMove(message.GetControlId(), message.GetParam1(), message.GetParam2());
      break;
    }
...
void CGUIWindow::FreeResources(bool forceUnload /*= FALSE */)
{
  if (! g_application.IsCurrentThread())
  {
    ...
    g_application.getApplicationMessenger().WindowFreeResources(this);
...
bool CGUIWindow::IsAnimating(ANIMATION_TYPE animType)
{
  ...
  else
  {
    for(int i = ANIM_TYPE_START; i < ANIM_TYPE_ANY; i++)
      if(CGUIControlGroup::IsAnimating((ANIMATION_TYPE)i))
        return true;
  }
  return false;
}
...
bool CGUIWindow::OnMove(int fromControl, int moveAction, int toControl)
{
  ...
  if (toControl != 0)
  {
    control = GetControl(toControl);
    nextControl = toControl;
  }
  else
  {
